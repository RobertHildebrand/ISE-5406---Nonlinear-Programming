...
const PRESETS = [
  {
    ...
    blurb:
      "A 2-variable max problem with two resource constraints. The LP optimum is fractional in both variables; B&B branches a few times before settling. Demonstrates branching, integer-feasible incumbents, and fathoming-by-bound.",
    varNames: ["x", "y"],
    nVars: 2,
    ...
    ipOptTeX: String.raw`(3, 1),\;\; \text{obj} = 19.5`,
  ...
  },
  {
    ...
    blurb:
      "A tighter 2-variable MILP where the root LP is fractional in both variables and B&B has to branch several times before convergence. Both branches survive long enough to see the dual bound drop in steps and the incumbent improve. Demonstrates fathoming-by-bound late in the search when a leftover branch's LP slips below the incumbent.",
    varNames: ["x", "y"],
    nVars: 2,
    objTeX: String.raw`\max\;\; 5\,x + 4\,y`,
    constraintsTeX: String.raw`6x + 5y \le 29,\;\; x + 2y \le 8,\;\; x, y \ge 0,\;\; x, y \in \mathbb{Z}`,
    lpOptTeX: String.raw`(18/7,\; 19/7) \approx (2.571,\; 2.714),\;\; \text{obj} \approx 23.71`,
    ipOptTeX: String.raw`(3, 2),\;\; \text{obj} = 23`,
    plotRange: { xmin: -0.3, xmax: 5.0, ymin: -0.3, ymax: 4.5, ticks: [0, 1, 2, 3, 4, 5] },
    constraintsForPlot: [
      { a: 6, b: 5, rhs: 29, label: "6x+5y=29" },
      { a: 1, b: 2, rhs: 8, label: "x+2y=8" },
    ],
    solver: makeSolver2D({
      c: [5, 4],
      lines: [
      ...
      ineq: (x, y) => 6 * x + 5 * y <= 29 + 1e-9 && x + 2 * y <= 8 + 1e-9,
      objNames: ["x", "y"],
    }),
  },
  {
...
export default function BranchBoundDemo() {
  ...
  return (
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "32px 24px 80px" }}>
      ...
      >
        {preset.nVars === 2 ? (
          <FeasibleRegionPlot node={selectedNode} incumbent={incumbent} preset={preset} />
        ) : (
...
// ============================================================
function FeasibleRegionPlot({ node, incumbent, preset }) {
  const W = 480, H = 480;
